...
export async function PUT(request) {
  try {
    ...
    const { phone, city, address, imageUrl } = updateProfileSchema.parse(body);
    ...
    if (Object.keys(updateData).length <= 1) { // Only updated_at
      return NextResponse.json({
        ...
        message: "Please provide at least one field to update (phone, city, address, or imageUrl)"
      }, { status: 400 });
    }
    ...
    return NextResponse.json({
      ...
      security_info: {
        ...
        editable_fields: ["phone", "city", "address", "imageUrl"]
      }
    });
